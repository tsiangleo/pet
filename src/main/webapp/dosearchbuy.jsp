   ...
   <td width="25%">宠物ID</td>
    ...
    <td width="25%">查看详情</td>
       ...
       <%
            ...
            String SQL ="select petID,petname,petprice from usertable,petinfo where account='"+username+"' and BaccountID=uaccountID";
...
<table>
        <tr> 
          <td nowrap>[<%=CurrPage%>/<%=CountPage %>] 每页3条 共<%=CountRow%>条记录 请输入页次 
          
     <div align="right">           
  <%if (CurrPage>1){%> 
            <a href="dosearchbuy.jsp?Page=<%=CurrPage-1%>&account=<%=username%>&id=<%=id%>">上一页</a> 
            <% 
          
          } 
          %> 
            <%if (CurrPage<CountPage){%> 
            <a href="dosearchbuy.jsp?Page=<%=CurrPage+1%>&account=<%=username%>&id=<%=id%>">下一页</a> 
            <% 
          
          } 
          %> <%if (CurrPage<CountPage){%>
            <a href="dosearchbuy.jsp?Page=<%=CountPage%>&account=<%=username%>&id=<%=id%>">最后一页</a> 
<%}%>
            </div></td> 
        </tr> 
      </table> 
